    ...
    try:
        ...
        try:
            ...
            # 当前浏览器的代理状态
                ...
                # 1. 代理成功标志：页面顶部蓝色区域显示有效网络IP地址。IP地址是由于数字与.组成的，所以有效网络IP地址一定要包含有数字；如果 IP 显示为 ---.---.---.---，则代理失败。
                # 2. 代理失败标志：页面顶部显示"代理失败"或页面顶部蓝色区域有出现 ---.---.---.--- 。

                # 注意：您的响应应遵循以下格式：
                # 代理成功返回 {"agent": "success"}
                # 代理失败返回 {"agent": "fail"}
                # 请勿包含任何其他信息。'''
                prompt = '''分析这张浏览器截图，请识别蓝色区域中，显示的内容是一个有效网络IP地址，还是 ---.---.---.--- 。

                注意：您的响应应遵循以下格式：
                显示的内容是一个有效网络IP地址，返回 {"agent": "success"}
                显示的内容是---.---.---.---，返回 {"agent": "fail"}
                显示的内容是"代理失败"，返回 {"agent": "fail"}
                请勿包含任何其他信息。
                '''
