...
def start_browser():
    try:
        ...
        try:
            ...
            image_paths = [img]
            prompt = '''这是一张浏览器最大化的截图，请判断浏览器打开了几个标签页。如何判断：根据图片最上面的一栏，“+” 字符前的标题个数，有几个标题，就有几个标签。如果标题只是单独一个特殊符号的是不能计算在内的，因为它不是标签，而是功能按钮。注意：您的响应应遵循以下格式：{"tab_count": 7}，例如：{"tab_count": 5}，其中 5 表示打开了5个标签页。请勿包含任何其他信息。'''
